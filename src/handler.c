...
Status  handle_browse_request(Request *r) {
    ...
    for (int i = 0; i < n; i++)
    {
        if (strcmp(".", entries[i]->d_name) != 0)
        {
            ...
            fprintf(r->stream, "<li class=\"list-group-item\"><a href=\"%s%s%s\">%s</a></li>\n", r->uri, sl, entries[i]->d_name, entries[i]->d_name);
...
Status  handle_error(Request *r, Status status) {
    ...
    /* Write HTML Description of Error*/
                                           // BOOTSTRAP LOGIC
                                           retHTML(r, "www/html/errorPre.html");
                                           char *alert;
                                           switch(status)
                                           {
                                           case HTTP_STATUS_OK:
                                           alert="alert-success"; break;
                                           case HTTP_STATUS_BAD_REQUEST: 
                                           alert="alert-danger"; break;
                                           case HTTP_STATUS_NOT_FOUND:
                                           alert="alert-info"; break;
                                           case HTTP_STATUS_INTERNAL_SERVER_ERROR:
                                           alert="alert-warning"; break;
                                           default: 
                                           alert="alert-success"; break;
                                           }
                                           fprintf(r->stream, "<div class=\"alert %s\"> <strong>Whoops!</strong> something went wrong.", alert);
